...
export default function Footer() {
  return (
    <footer className="bg-paper-2 text-ink-800 border-t border-mist">
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8 pt-16 pb-8">
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-10 lg:gap-8 pb-12 border-b border-mist">
          <div className="col-span-2">
            <Logo variant="dark" />
            <p className="mt-5 text-sm text-slate leading-relaxed max-w-[280px]">
            ...
            <div className="mt-6 flex items-center gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-9 h-9 rounded-full border border-mist bg-white flex items-center justify-center text-slate hover:text-electric-2 hover:border-electric transition-colors"
          ...
          <div>
            <p className="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-slate-2 mb-4">Contact</p>
            <ul className="space-y-2.5 text-sm text-slate">
              <li>contact@doxamed.com</li>
              <li>Paris, France</li>
              <li className="pt-1">
                <Link href="/contact" className="text-electric-2 font-medium hover:text-electric transition-colors">
        ...
        <div className="pt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2">
            {FOOTER_LINKS.legal.map((l) => (
              <Link key={l.href} href={l.href} className="text-xs text-slate-2 hover:text-slate transition-colors">
                {l.label}
              </Link>
            ))}
          </div>
...
function FooterCol({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div>
      <p className="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-slate-2 mb-4">{title}</p>
