...
export default function Home() {
  return (
    <>
      {/* HERO */}
      <section className="relative bg-ink-800 text-white overflow-hidden pb-16 sm:pb-20">
        <div className="relative overflow-hidden">
          ...
          <span className="hidden lg:block absolute top-24 right-[8%] w-40 h-40 rounded-full border border-dashed border-electric/25 pointer-events-none" />
          <span className="hidden lg:block absolute top-40 right-[14%] w-4 h-4 rounded-full bg-electric/30 pointer-events-none" />
        ...
        {/* Stats dock — floats over the hero/next-section seam rather than
            sitting inline as a plain divided row (Propale 1's treatment). */}
        <Section className="relative -mt-16 sm:-mt-20">
          <div className="rounded-2xl border border-white/10 bg-ink-700/90 backdrop-blur-sm shadow-2xl shadow-black/30 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 divide-x divide-y sm:divide-y-0 divide-white/10">
            {GLOBAL_STATS.map((s, i) => (
              <Reveal key={s.label} delay={i * 60} from="up">
                <div className="px-5 py-6">
                  <div className="font-mono-num text-xl sm:text-2xl font-semibold text-electric-light">
